...
class UserResourceTestCase(unittest.TestCase):
    def setUp(self):
        ...
        content = Content(title='test-title', content="test-content", user_id=user_id)
    ...
    def test_contents_get_by_content_id(self):
        ...
        response = client.get('/content/' + str(content.id))
    ...
    def test_contents_get_by_user_id(self):
        ...
        client = self.app.test_client()
        response = client.get('/contents/' + str(user.id))
        self.assertEqual(response.status_code, 200)
        content_dict = json.loads(response.data)
        self.assertEqual(len(content_dict), 1)
        self.assertEqual(content_dict[0]['title'], content.title)
        self.assertEqual(content_dict[0]['content'], content.content)
        self.assertEqual(content_dict[0]['created_at'], content.created_at.strftime('%Y-%m-%dT%H:%M:%S'))
        self.assertEqual(content_dict[0]['updated_at'], content.updated_at.strftime('%Y-%m-%dT%H:%M:%S'))

    def test_contents_post(self):
        user = db.session.query(User).filter_by(user_name='janedoe').first()
        js = '{"user_id": %d, "title": "sample", "content": "foobarhoge"}' % user.id
        client = self.app.test_client()
        response = client.post('/contents', data=js, content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_contents_put(self):
        ...
        client = self.app.test_client()
        response = client.put('/content/%d' % content.id, data=js, content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_contents_delete(self):
        content = db.session.query(Content).filter_by(title='test-title').first()
        client = self.app.test_client()
        response = client.delete('/content/%d' % content.id)
        self.assertEqual(response.status_code, 200)
